import {
  TooltipModule,
  init_primeng_tooltip
} from "./chunk-WGD7ETYN.js";
import {
  Component,
  Input,
  __decorate,
  init_core,
  init_tslib_es6,
  input
} from "./chunk-56THSHHT.js";
import {
  __esm
} from "./chunk-TTULUY32.js";

// angular:jit:template:src/app/components/social-media-link/social-media-link.html
var social_media_link_default;
var init_social_media_link = __esm({
  "angular:jit:template:src/app/components/social-media-link/social-media-link.html"() {
    social_media_link_default = `<a
  [href]="url()"
  target="_blank"
  [pTooltip]="provider().charAt(0).toUpperCase() + provider().slice(1)"
  tooltipPosition="top"
>
  @switch (provider().toLowerCase()) { @case ('github') {
  <svg
    github-icon
    class="hover:opacity-80 transition-opacity size-6 lg:size-8"
  ></svg>
  } @case ('twitter') {
  <svg
    twitter-icon
    class="hover:opacity-80 transition-opacity size-6 lg:size-8"
  ></svg>

  } @case ('facebook') {
  <svg
    facebook-icon
    class="hover:opacity-80 transition-opacity size-6 lg:size-8"
  ></svg>

  } @case ('instagram') {
  <svg
    instagram-icon
    class="hover:opacity-80 transition-opacity size-6 lg:size-8"
  ></svg>

  } @case ('twitch') {
  <svg
    twitch-icon
    class="hover:opacity-80 transition-opacity size-6 lg:size-8"
  ></svg>

  } @case ('linkedin') {

  <svg
    linkedin-icon
    class="hover:opacity-80 transition-opacity size-6 lg:size-8"
  ></svg>
  } @case ('mastodon') {

  <svg
    mastodon-icon
    class="hover:opacity-80 transition-opacity size-6 lg:size-8"
  ></svg>
  } @case ('reddit') {
  <svg
    reddit-icon
    class="hover:opacity-80 transition-opacity size-6 lg:size-8"
  ></svg>

  } @case ('bluesky') {
  <svg
    bluesky-icon
    class="hover:opacity-80 transition-opacity size-6 lg:size-8"
  ></svg>

  } @default {

  <svg
    default-icon
    class="hover:opacity-80 transition-opacity size-6 lg:size-8"
  ></svg>
  } }
</a>
`;
  }
});

// src/app/icons/github.ts
var GitHubIcon;
var init_github = __esm({
  "src/app/icons/github.ts"() {
    "use strict";
    init_tslib_es6();
    init_core();
    GitHubIcon = class GitHubIcon2 {
    };
    GitHubIcon = __decorate([
      Component({
        selector: "svg[github-icon]",
        imports: [],
        template: `<svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 640 640"
    fill="#92898a"
  >
    <path
      d="M237.9 461.4C237.9 463.4 235.6 465 232.7 465C229.4 465.3 227.1 463.7 227.1 461.4C227.1 459.4 229.4 457.8 232.3 457.8C235.3 457.5 237.9 459.1 237.9 461.4zM206.8 456.9C206.1 458.9 208.1 461.2 211.1 461.8C213.7 462.8 216.7 461.8 217.3 459.8C217.9 457.8 216 455.5 213 454.6C210.4 453.9 207.5 454.9 206.8 456.9zM251 455.2C248.1 455.9 246.1 457.8 246.4 460.1C246.7 462.1 249.3 463.4 252.3 462.7C255.2 462 257.2 460.1 256.9 458.1C256.6 456.2 253.9 454.9 251 455.2zM316.8 72C178.1 72 72 177.3 72 316C72 426.9 141.8 521.8 241.5 555.2C254.3 557.5 258.8 549.6 258.8 543.1C258.8 536.9 258.5 502.7 258.5 481.7C258.5 481.7 188.5 496.7 173.8 451.9C173.8 451.9 162.4 422.8 146 415.3C146 415.3 123.1 399.6 147.6 399.9C147.6 399.9 172.5 401.9 186.2 425.7C208.1 464.3 244.8 453.2 259.1 446.6C261.4 430.6 267.9 419.5 275.1 412.9C219.2 406.7 162.8 398.6 162.8 302.4C162.8 274.9 170.4 261.1 186.4 243.5C183.8 237 175.3 210.2 189 175.6C209.9 169.1 258 202.6 258 202.6C278 197 299.5 194.1 320.8 194.1C342.1 194.1 363.6 197 383.6 202.6C383.6 202.6 431.7 169 452.6 175.6C466.3 210.3 457.8 237 455.2 243.5C471.2 261.2 481 275 481 302.4C481 398.9 422.1 406.6 366.2 412.9C375.4 420.8 383.2 435.8 383.2 459.3C383.2 493 382.9 534.7 382.9 542.9C382.9 549.4 387.5 557.3 400.2 555C500.2 521.8 568 426.9 568 316C568 177.3 455.5 72 316.8 72zM169.2 416.9C167.9 417.9 168.2 420.2 169.9 422.1C171.5 423.7 173.8 424.4 175.1 423.1C176.4 422.1 176.1 419.8 174.4 417.9C172.8 416.3 170.5 415.6 169.2 416.9zM158.4 408.8C157.7 410.1 158.7 411.7 160.7 412.7C162.3 413.7 164.3 413.4 165 412C165.7 410.7 164.7 409.1 162.7 408.1C160.7 407.5 159.1 407.8 158.4 408.8zM190.8 444.4C189.2 445.7 189.8 448.7 192.1 450.6C194.4 452.9 197.3 453.2 198.6 451.6C199.9 450.3 199.3 447.3 197.3 445.4C195.1 443.1 192.1 442.8 190.8 444.4zM179.4 429.7C177.8 430.7 177.8 433.3 179.4 435.6C181 437.9 183.7 438.9 185 437.9C186.6 436.6 186.6 434 185 431.7C183.6 429.4 181 428.4 179.4 429.7z"
    />
  </svg>`
      })
    ], GitHubIcon);
  }
});

// src/app/icons/facebook.ts
var FacebookIcon;
var init_facebook = __esm({
  "src/app/icons/facebook.ts"() {
    "use strict";
    init_tslib_es6();
    init_core();
    FacebookIcon = class FacebookIcon2 {
    };
    FacebookIcon = __decorate([
      Component({
        selector: "svg[facebook-icon]",
        imports: [],
        template: `<svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 640 640"
    fill="#92898a"
  >
    <path
      d="M576 320C576 178.6 461.4 64 320 64C178.6 64 64 178.6 64 320C64 440 146.7 540.8 258.2 568.5L258.2 398.2L205.4 398.2L205.4 320L258.2 320L258.2 286.3C258.2 199.2 297.6 158.8 383.2 158.8C399.4 158.8 427.4 162 438.9 165.2L438.9 236C432.9 235.4 422.4 235 409.3 235C367.3 235 351.1 250.9 351.1 292.2L351.1 320L434.7 320L420.3 398.2L351 398.2L351 574.1C477.8 558.8 576 450.9 576 320z"
    />
  </svg>`
      })
    ], FacebookIcon);
  }
});

// src/app/icons/instagram.ts
var InstagramIcon;
var init_instagram = __esm({
  "src/app/icons/instagram.ts"() {
    "use strict";
    init_tslib_es6();
    init_core();
    InstagramIcon = class InstagramIcon2 {
    };
    InstagramIcon = __decorate([
      Component({
        selector: "svg[instagram-icon]",
        imports: [],
        template: `<svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 640 640"
    fill="#92898a"
  >
    <path
      d="M320.3 205C256.8 204.8 205.2 256.2 205 319.7C204.8 383.2 256.2 434.8 319.7 435C383.2 435.2 434.8 383.8 435 320.3C435.2 256.8 383.8 205.2 320.3 205zM319.7 245.4C360.9 245.2 394.4 278.5 394.6 319.7C394.8 360.9 361.5 394.4 320.3 394.6C279.1 394.8 245.6 361.5 245.4 320.3C245.2 279.1 278.5 245.6 319.7 245.4zM413.1 200.3C413.1 185.5 425.1 173.5 439.9 173.5C454.7 173.5 466.7 185.5 466.7 200.3C466.7 215.1 454.7 227.1 439.9 227.1C425.1 227.1 413.1 215.1 413.1 200.3zM542.8 227.5C541.1 191.6 532.9 159.8 506.6 133.6C480.4 107.4 448.6 99.2 412.7 97.4C375.7 95.3 264.8 95.3 227.8 97.4C192 99.1 160.2 107.3 133.9 133.5C107.6 159.7 99.5 191.5 97.7 227.4C95.6 264.4 95.6 375.3 97.7 412.3C99.4 448.2 107.6 480 133.9 506.2C160.2 532.4 191.9 540.6 227.8 542.4C264.8 544.5 375.7 544.5 412.7 542.4C448.6 540.7 480.4 532.5 506.6 506.2C532.8 480 541 448.2 542.8 412.3C544.9 375.3 544.9 264.5 542.8 227.5zM495 452C487.2 471.6 472.1 486.7 452.4 494.6C422.9 506.3 352.9 503.6 320.3 503.6C287.7 503.6 217.6 506.2 188.2 494.6C168.6 486.8 153.5 471.7 145.6 452C133.9 422.5 136.6 352.5 136.6 319.9C136.6 287.3 134 217.2 145.6 187.8C153.4 168.2 168.5 153.1 188.2 145.2C217.7 133.5 287.7 136.2 320.3 136.2C352.9 136.2 423 133.6 452.4 145.2C472 153 487.1 168.1 495 187.8C506.7 217.3 504 287.3 504 319.9C504 352.5 506.7 422.6 495 452z"
    />
  </svg>`
      })
    ], InstagramIcon);
  }
});

// src/app/icons/twitch.ts
var TwitchIcon;
var init_twitch = __esm({
  "src/app/icons/twitch.ts"() {
    "use strict";
    init_tslib_es6();
    init_core();
    TwitchIcon = class TwitchIcon2 {
    };
    TwitchIcon = __decorate([
      Component({
        selector: "svg[twitch-icon]",
        imports: [],
        template: `<svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 640 640"
    fill="#92898a"
  >
    <path
      d="M455.4 167.5L416.8 167.5L416.8 277.2L455.4 277.2L455.4 167.5zM349.2 167L310.6 167L310.6 276.8L349.2 276.8L349.2 167zM185 64L88.5 155.4L88.5 484.6L204.3 484.6L204.3 576L300.8 484.6L378.1 484.6L551.9 320L551.9 64L185 64zM513.3 301.8L436.1 374.9L358.9 374.9L291.3 438.9L291.3 374.9L204.4 374.9L204.4 100.6L513.3 100.6L513.3 301.8z"
    />
  </svg>`
      })
    ], TwitchIcon);
  }
});

// src/app/icons/linkedin.ts
var LinkedInIcon;
var init_linkedin = __esm({
  "src/app/icons/linkedin.ts"() {
    "use strict";
    init_tslib_es6();
    init_core();
    LinkedInIcon = class LinkedInIcon2 {
    };
    LinkedInIcon = __decorate([
      Component({
        selector: "svg[linkedin-icon]",
        imports: [],
        template: `<svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 640 640"
    fill="#92898a"
  >
    <path
      d="M512 96L127.9 96C110.3 96 96 110.5 96 128.3L96 511.7C96 529.5 110.3 544 127.9 544L512 544C529.6 544 544 529.5 544 511.7L544 128.3C544 110.5 529.6 96 512 96zM231.4 480L165 480L165 266.2L231.5 266.2L231.5 480L231.4 480zM198.2 160C219.5 160 236.7 177.2 236.7 198.5C236.7 219.8 219.5 237 198.2 237C176.9 237 159.7 219.8 159.7 198.5C159.7 177.2 176.9 160 198.2 160zM480.3 480L413.9 480L413.9 376C413.9 351.2 413.4 319.3 379.4 319.3C344.8 319.3 339.5 346.3 339.5 374.2L339.5 480L273.1 480L273.1 266.2L336.8 266.2L336.8 295.4L337.7 295.4C346.6 278.6 368.3 260.9 400.6 260.9C467.8 260.9 480.3 305.2 480.3 362.8L480.3 480z"
    />
  </svg>`
      })
    ], LinkedInIcon);
  }
});

// src/app/icons/mastodon.ts
var MastodonIcon;
var init_mastodon = __esm({
  "src/app/icons/mastodon.ts"() {
    "use strict";
    init_tslib_es6();
    init_core();
    MastodonIcon = class MastodonIcon2 {
    };
    MastodonIcon = __decorate([
      Component({
        selector: "svg[mastodon-icon]",
        imports: [],
        template: `<svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 640 640"
    fill="#92898a"
  >
   <path
      d="M529 243.1C529 145.9 465.3 117.4 465.3 117.4C402.8 88.7 236.7 89 174.8 117.4C174.8 117.4 111.1 145.9 111.1 243.1C111.1 358.8 104.5 502.5 216.7 532.2C257.2 542.9 292 545.2 320 543.6C370.8 540.8 399.3 525.5 399.3 525.5L397.6 488.6C397.6 488.6 361.3 500 320.5 498.7C280.1 497.3 237.5 494.3 230.9 444.7C230.3 440.1 230 435.4 230 430.8C315.6 451.7 388.7 439.9 408.7 437.5C464.8 430.8 513.7 396.2 519.9 364.6C529.7 314.8 528.9 243.1 528.9 243.1zM453.9 368.3L407.3 368.3L407.3 254.1C407.3 204.4 343.3 202.5 343.3 261L343.3 323.5L297 323.5L297 261C297 202.5 233 204.4 233 254.1L233 368.3L186.3 368.3C186.3 246.2 181.1 220.4 204.7 193.3C230.6 164.4 284.5 162.5 308.5 199.4L320.1 218.9L331.7 199.4C355.8 162.3 409.8 164.6 435.5 193.3C459.2 220.6 453.9 246.3 453.9 368.3L453.9 368.3z"
    />
  </svg>`
      })
    ], MastodonIcon);
  }
});

// src/app/icons/reddit.ts
var RedditIcon;
var init_reddit = __esm({
  "src/app/icons/reddit.ts"() {
    "use strict";
    init_tslib_es6();
    init_core();
    RedditIcon = class RedditIcon2 {
    };
    RedditIcon = __decorate([
      Component({
        selector: "svg[reddit-icon]",
        imports: [],
        template: `<svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 640 640"
    fill="#92898a"
  >
   <path
      d="M64 320C64 178.6 178.6 64 320 64C461.4 64 576 178.6 576 320C576 461.4 461.4 576 320 576L101.1 576C87.4 576 80.6 559.5 90.2 549.8L139 501C92.7 454.7 64 390.7 64 320zM413.6 217.6C437.2 217.6 456.3 198.5 456.3 174.9C456.3 151.3 437.2 132.2 413.6 132.2C393 132.2 375.8 146.8 371.8 166.2C337.3 169.9 310.4 199.2 310.4 234.6L310.4 234.8C272.9 236.4 238.6 247.1 211.4 263.9C201.3 256.1 188.6 251.4 174.9 251.4C141.9 251.4 115.1 278.2 115.1 311.2C115.1 335.2 129.2 355.8 149.5 365.3C151.5 434.7 227.1 490.5 320.1 490.5C413.1 490.5 488.8 434.6 490.7 365.2C510.9 355.6 524.8 335 524.8 311.2C524.8 278.2 498 251.4 465 251.4C451.3 251.4 438.7 256 428.6 263.8C401.2 246.8 366.5 236.1 328.6 234.7L328.6 234.5C328.6 209.1 347.5 188 372 184.6C376.4 203.4 393.3 217.4 413.5 217.4L413.6 217.6zM241.1 310.9C257.8 310.9 270.6 328.5 269.6 350.2C268.6 371.9 256.1 379.8 239.3 379.8C222.5 379.8 207.9 371 208.9 349.3C209.9 327.6 224.3 311 241 311L241.1 310.9zM431.2 349.2C432.2 370.9 417.5 379.7 400.8 379.7C384.1 379.7 371.5 371.8 370.5 350.1C369.5 328.4 382.3 310.8 399 310.8C415.7 310.8 430.2 327.4 431.1 349.1L431.2 349.2zM383.1 405.9C372.8 430.5 348.5 447.8 320.1 447.8C291.7 447.8 267.4 430.5 257.1 405.9C255.9 403 257.9 399.7 261 399.4C279.4 397.5 299.3 396.5 320.1 396.5C340.9 396.5 360.8 397.5 379.2 399.4C382.3 399.7 384.3 403 383.1 405.9z"
    />
  </svg>`
      })
    ], RedditIcon);
  }
});

// src/app/icons/bluesky.ts
var BlueskyIcon;
var init_bluesky = __esm({
  "src/app/icons/bluesky.ts"() {
    "use strict";
    init_tslib_es6();
    init_core();
    BlueskyIcon = class BlueskyIcon2 {
    };
    BlueskyIcon = __decorate([
      Component({
        selector: "svg[bluesky-icon]",
        imports: [],
        template: `<svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 640 640"
    fill="#92898a"
  >
   <path
      d="M439.8 358.7C436.5 358.3 433.1 357.9 429.8 357.4C433.2 357.8 436.5 358.3 439.8 358.7zM320 291.1C293.9 240.4 222.9 145.9 156.9 99.3C93.6 54.6 69.5 62.3 53.6 69.5C35.3 77.8 32 105.9 32 122.4C32 138.9 41.1 258 47 277.9C66.5 343.6 136.1 365.8 200.2 358.6C203.5 358.1 206.8 357.7 210.2 357.2C206.9 357.7 203.6 358.2 200.2 358.6C106.3 372.6 22.9 406.8 132.3 528.5C252.6 653.1 297.1 501.8 320 425.1C342.9 501.8 369.2 647.6 505.6 528.5C608 425.1 533.7 372.5 439.8 358.6C436.5 358.2 433.1 357.8 429.8 357.3C433.2 357.7 436.5 358.2 439.8 358.6C503.9 365.7 573.4 343.5 593 277.9C598.9 258 608 139 608 122.4C608 105.8 604.7 77.7 586.4 69.5C570.6 62.4 546.4 54.6 483.2 99.3C417.1 145.9 346.1 240.4 320 291.1z"
    />
  </svg>`
      })
    ], BlueskyIcon);
  }
});

// src/app/icons/twitter.ts
var TwitterIcon;
var init_twitter = __esm({
  "src/app/icons/twitter.ts"() {
    "use strict";
    init_tslib_es6();
    init_core();
    TwitterIcon = class TwitterIcon2 {
    };
    TwitterIcon = __decorate([
      Component({
        selector: "svg[twitter-icon]",
        imports: [],
        template: ` <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 640 640"
    fill="#92898a"
  >
    <path
      d="M160 96C124.7 96 96 124.7 96 160L96 480C96 515.3 124.7 544 160 544L480 544C515.3 544 544 515.3 544 480L544 160C544 124.7 515.3 96 480 96L160 96zM447.3 263.3C447.3 350 381.3 449.9 260.7 449.9C223.5 449.9 189 439.1 160 420.5C165.3 421.1 170.4 421.3 175.8 421.3C206.5 421.3 234.7 410.9 257.2 393.3C228.4 392.7 204.2 373.8 195.9 347.8C206 349.3 215.1 349.3 225.5 346.6C195.5 340.5 173 314.1 173 282.2L173 281.4C181.7 286.3 191.9 289.3 202.6 289.7C193.6 283.7 186.2 275.6 181.1 266.1C176 256.6 173.3 245.9 173.4 235.1C173.4 222.9 176.6 211.7 182.3 202C214.6 241.8 263.1 267.8 317.5 270.6C308.2 226.1 341.5 190 381.5 190C400.4 190 417.4 197.9 429.4 210.7C444.2 207.9 458.4 202.4 471 194.9C466.1 210.1 455.8 222.9 442.2 231C455.4 229.6 468.2 225.9 480 220.8C471.1 233.9 459.9 245.5 447.1 254.8C447.3 257.6 447.3 260.5 447.3 263.3z"
    />
  </svg>`
      })
    ], TwitterIcon);
  }
});

// src/app/icons/default.ts
var DefaultIcon;
var init_default = __esm({
  "src/app/icons/default.ts"() {
    "use strict";
    init_tslib_es6();
    init_core();
    DefaultIcon = class DefaultIcon2 {
    };
    DefaultIcon = __decorate([
      Component({
        selector: "svg[default-icon]",
        imports: [],
        template: `<svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 640 640"
    fill="#92898a"
  >
    <path
      d="M451.5 160C434.9 160 418.8 164.5 404.7 172.7C388.9 156.7 370.5 143.3 350.2 133.2C378.4 109.2 414.3 96 451.5 96C537.9 96 608 166 608 252.5C608 294 591.5 333.8 562.2 363.1L491.1 434.2C461.8 463.5 422 480 380.5 480C294.1 480 224 410 224 323.5C224 322 224 320.5 224.1 319C224.6 301.3 239.3 287.4 257 287.9C274.7 288.4 288.6 303.1 288.1 320.8C288.1 321.7 288.1 322.6 288.1 323.4C288.1 374.5 329.5 415.9 380.6 415.9C405.1 415.9 428.6 406.2 446 388.8L517.1 317.7C534.4 300.4 544.2 276.8 544.2 252.3C544.2 201.2 502.8 159.8 451.7 159.8zM307.2 237.3C305.3 236.5 303.4 235.4 301.7 234.2C289.1 227.7 274.7 224 259.6 224C235.1 224 211.6 233.7 194.2 251.1L123.1 322.2C105.8 339.5 96 363.1 96 387.6C96 438.7 137.4 480.1 188.5 480.1C205 480.1 221.1 475.7 235.2 467.5C251 483.5 269.4 496.9 289.8 507C261.6 530.9 225.8 544.2 188.5 544.2C102.1 544.2 32 474.2 32 387.7C32 346.2 48.5 306.4 77.8 277.1L148.9 206C178.2 176.7 218 160.2 259.5 160.2C346.1 160.2 416 230.8 416 317.1C416 318.4 416 319.7 416 321C415.6 338.7 400.9 352.6 383.2 352.2C365.5 351.8 351.6 337.1 352 319.4C352 318.6 352 317.9 352 317.1C352 283.4 334 253.8 307.2 237.5z"
    />
  </svg>`
      })
    ], DefaultIcon);
  }
});

// src/app/icons/index.ts
var init_icons = __esm({
  "src/app/icons/index.ts"() {
    "use strict";
    init_github();
    init_facebook();
    init_instagram();
    init_twitch();
    init_linkedin();
    init_mastodon();
    init_reddit();
    init_bluesky();
    init_twitter();
    init_default();
  }
});

// src/app/components/social-media-link/social-media-link.ts
var SocialMediaLink;
var init_social_media_link2 = __esm({
  "src/app/components/social-media-link/social-media-link.ts"() {
    "use strict";
    init_tslib_es6();
    init_social_media_link();
    init_core();
    init_primeng_tooltip();
    init_icons();
    init_core();
    SocialMediaLink = class SocialMediaLink2 {
      url = input.required();
      provider = input.required();
      static propDecorators = {
        url: [{ type: Input, args: [{ isSignal: true, alias: "url", required: true, transform: void 0 }] }],
        provider: [{ type: Input, args: [{ isSignal: true, alias: "provider", required: true, transform: void 0 }] }]
      };
    };
    SocialMediaLink = __decorate([
      Component({
        selector: "app-social-media-link",
        imports: [
          TooltipModule,
          GitHubIcon,
          FacebookIcon,
          InstagramIcon,
          TwitchIcon,
          TwitterIcon,
          LinkedInIcon,
          MastodonIcon,
          BlueskyIcon,
          RedditIcon,
          DefaultIcon
        ],
        template: social_media_link_default
      })
    ], SocialMediaLink);
  }
});

export {
  SocialMediaLink,
  init_social_media_link2 as init_social_media_link
};
//# sourceMappingURL=chunk-E2CWIQEH.js.map
